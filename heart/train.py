...
def prepare_val_features_for_predict(train_features: pd.DataFrame, val_features: pd.DataFrame):
    ...
    Returns:
        pd.DataFrame: Prepared validation features.
    """
...
def train_pipeline(params: TrainingPipelineConfig) -> None:
    """End-to-end train pipeline function.

    Args:
        params (TrainingPipelineConfig): Training pipeline configuration.
    """
    logger.info("Starting pipeline")
    data = read_data(params.dataset.input_data_path)
    logger.info("data.shape is %s", data.shape)

    train_df, test_df = split_train_test_data(dataset=data, test_size=0.25, random_state=42)
    ...
    val_features_prepared = prepare_val_features_for_predict(train_features, test_features)
...
def main(cfg: TrainingPipelineConfig) -> None:
    ...
    Args:
        cfg (TrainingPipelineConfig): Training pipeline configuration.
    """
    train_pipeline(cfg)
